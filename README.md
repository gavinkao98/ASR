...
在任何 Windows 應用程式裡，用說話取代打字——完全在你自己的電腦上執行，不用連網、也不會把你的聲音送到雲端（第一次安裝時下載模型那次除外）。預設辨識引擎是聯發科的 **Breeze-ASR-25**，針對繁體中文調校過；也內建一顆可自由切換的備用引擎 **Qwen3**。
...
如果不想每次都跑到專案資料夾找 `啟動語音輸入.vbs`，可以建一個桌面捷徑：在專案資料夾空白處按住 **Shift 鍵再按右鍵**，選「在這裡開啟 PowerShell 視窗」，貼上以下指令後按 Enter：
...
2. **下載辨識模型**：預設引擎 Breeze-ASR-25 加上標點模型，總共約 **1.9GB**，需要網路連線，依網速可能要幾分鐘，請耐心等待、不要中途關閉視窗。
...
| 分頁 | 用途 |
|---|---|
| **一般** | 開機自動啟動、提示音開關、貼上方式、選擇要用哪個麥克風裝置 |
...
**防毒軟體跳出警告，說這是可疑程式**
這個工具需要「全域監聽鍵盤」才能偵測你按住 CapsLock，這個行為看起來很像鍵盤側錄程式，容易被防毒軟體誤判。請把整個專案資料夾加入防毒軟體的白名單（允許清單），誤報就會消失。
...
貼上文字的那一瞬間，工具會短暫「借用」一下你的剪貼簿，貼完後會自動把你原本複製的內容還原回去——但這個還原**只支援文字**。如果在使用語音輸入之前，剪貼簿裡放的是圖片等非文字內容，就沒辦法還原，請留意這一點。
